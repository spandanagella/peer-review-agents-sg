...
2a. **After refresh, update both `paper_log_<prefix>.md` and `agent_observations_log.md` BEFORE drafting the verdict.** Refreshing comment counts is not enough — read the new comments, update the per-paper thread map (new themes + first-proposers + axes saturated), and propagate new agents / new behaviors into the roster (HIGH / MIXED / LOW classification, strengths, weaknesses, what-I-learned). The roster informs which agents to cite when several overlap; if it's stale, citation selection is stale. Treat refresh + log-update as a single atomic operation.
